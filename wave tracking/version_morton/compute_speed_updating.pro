...
FUNCTION compute_speed_updating, velocityTrack, sampling, cadence, numberLagValues, $
                        debug=debug, ret=ret, force_zero=force_zero


s=size(velocityTrack)
nt=s(1)
trackLength=s(2)
IF trackLength mod 2 EQ 0 THEN trackLength=trackLength-1 ;deals with even length tracks
trackCenter=trackLength/2

lag=indgen(numberLagValues)-fix(numberLagValues/2)
range = 19 < trackLength  ;number of points along track for initial guess (make odd)


lagEstimates=trackCrossCorrelate(velocityTrack,trackCenter,range)
inversePropSpeed=[0.,median(deriv(lagEstimates))] 
lowNoiseCentralSeries=fltarr(nt)
time=rebin(findgen(nt),nt,trackLength)
seriesShift=time+transpose(rebin(inversePropSpeed[1]*(findgen(trackLength)-trackLength/2)+inversePropSpeed[0],trackLength,nt))

FOR i=0,trackLength-1 DO lowNoiseCentralSeries=temporary(lowNoiseCentralSeries)+interpolate(velocityTrack[*,i],seriesShift[i])

lowNoiseCentralSeries=lowNoiseCentralSeries/float(trackLength)


IF keyword_set(debug) THEN $
 plot,lag,/nodata,xr=[-numberLagValues/2.,numberLagValues/2.],yr=[-trackLength/4.-1,trackLength/4.+1],ysty=1,xsty=1,xtit='Lag (Time Steps)',$
 	ytit='Correlation',chars=1.5

lagEstimates=trackCrossCorrelate(velocityTrack,trackCenter,trackLength,lowNoiseCentralSeries=lowNoiseCentralSeries,debug=debug)
...
shifts=deriv(lagEstimates)
init_guess=lag*median(shifts)

pdistance = fltarr(trackLength)
FOR ii=0,trackLength-1 DO pdistance[ii] = pnt_line([lag[ii],lagEstimates[ii]], [lag[0],init_guess[0]], [lag[trackLength-1],init_guess[trackLength-1]])
...
param = mpfitfun('lin_func_fit', lag[good_p], lagEstimates[good_p], err, parinfo=parinfo, /quiet,xtol=1e-4)
...
;##############################################################
cor_val=fltarr(trackLength)
tstep_tolerance = 4
for i=0,trackLength-1 do begin
  ccor=c_correlate(col,vel(*,i),lag)
  lo_l = i_yfit[i]-tstep_tolerance+trackLength/2. ; lower limit
  up_l = i_yfit[i]+tstep_tolerance+trackLength/2. ; upper limit
  if (lo_l gt 1 and up_l lt numberLagValues-2) then begin 
    m=max(ccor[lo_l:up_l],imax)
    imax=imax+i_yfit[i]-tstep_tolerance+trackLength/2.
    if keyword_set(debug) then plots, imax-numberLagValues/2., m -trackLength/4. + i*0.5, psym = 4, color=100
    if keyword_set(debug) then plots, lo_l-numberLagValues/2., ccor[lo_l] -trackLength/4. + i*0.5, psym = 5, color=250
    if keyword_set(debug) then plots, up_l-numberLagValues/2., ccor[up_l] -trackLength/4. + i*0.5, psym = 5, color=250

  endif else begin
   m=max(ccor,imax)
   imax=1 > imax < (numberLagValues-2)
   if keyword_set(debug) then plots, imax-numberLagValues/2., m -trackLength/4. + i*0.5, psym = 4, color=185
  endelse
  lagEstimates(i)=parabola(lag(imax-1:imax+1),1.-ccor(imax-1:imax+1))
  cor_val(i)=m
if keyword_set(debug) then plots, lagEstimates[i], m -trackLength/4. + i*0.5,psym = 5,color=200
endfor



data = lagEstimates*cadence

xfit=(findgen(trackLength)-fix(trackLength/2))*sampling
if keyword_set(debug) then begin
 plot,xfit,data,psym=1,xtit='Position Along Track (Mm)',ytit='Lag (s)',chars=1.5,$
     xr=[-trackLength/2,trackLength/2]*sampling*1.5,xsty=1,yr=[-trackLength,trackLength]*4,ysty=1
;********
...
;##############################################################
shifts=deriv(lagEstimates)*cadence/sampling
init_guess=xfit*median(shifts)

;first iteration
pdistance = fltarr(trackLength)
for ii=0,trackLength-1 do begin
 pdistance[ii] = pnt_line([xfit[ii],data[ii]], [xfit[0],init_guess[0]], [xfit[trackLength-1],init_guess[trackLength-1]])
endfor
...
pdistance = fltarr(trackLength)
;for ii=0,trackLength-1 do begin
; pdistance[ii] = pnt_line([xfit[ii],data[ii]], [xfit[0],i_yfit[0]], [xfit[trackLength-1],i_yfit[trackLength-1]])
;endfor
...
FUNCTION trackCrossCorrelate,velocityTrack,trackCenter,range,lowNoiseCentralSeries=lowNoiseCentralSeries,debug=debug
  ;Cross-correlate rows to central row in time-distance map
  crossCorrMaximums=fltarr(range)
  IF n_elements(lowNoiseCentralSeries) EQ 0 THEN velocityTrackCenter=reform(velocityTrack(*,trackCenter)) $
                                            ELSE velocityTrackCenter=lowNoiseCentralSeries
  FOR i=-range/2,range/2 DO BEGIN
    crossCorrValues=c_correlate(velocityTrackCenter,velocityTrack(*,trackCenter+i),lag)
    m=max(crossCorrValues,maxLag)
    maxLag=1 > maxLag < (numberLagValues-2)
    lagEstimates(i+range/2)=parabola(lag(maxLag-1:maxLag+1),1.-crossCorrValues(maxLag-1:maxLag+1))
    IF keyword_set(debug) THEN oplot,lag,crossCorrValues-trackLength/4.+i*0.5, col=50 
  ENDFOR
